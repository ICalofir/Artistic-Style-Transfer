...
class StyleTransfer():
  ...
  def build(self):
    ...
    self.image_decoded = tf.placeholder(tf.uint8, [self.content_img_height,
                                   self.content_img_width, self.content_img_channels])
    self.fname = tf.placeholder(tf.string)
    self.enc = tf.image.encode_png(self.image_decoded)
    self.fwrite = tf.write_file(self.fname, self.enc)
    ...
    with tf.Session() as sess:
      ...
      for i in range(self.num_iters):
        ...
        if i % 20 == 0:
          ...
          img_dec = ut.denormalize_img(out_img[0])
          img_dec = cv2.cvtColor(img_dec, cv2.COLOR_BGR2RGB)
          sess.run(self.fwrite,
                   feed_dict={self.image_decoded: img_dec,
                              self.fname: output_img_path + '/img' + str(i) + '.png'})
