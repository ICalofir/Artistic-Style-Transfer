      ...
      for learning_rate in learning_rate_v:
        for alfa in alfa_v:
          for beta in beta_v:
            ...
            # image_content_decoded = tf.image.decode_jpeg(tf.read_file(content_img_path), channels=3)
            # image_style_decoded = tf.image.decode_jpeg(tf.read_file(style_img_path), channels=3)
            image_content_decoded = tf.read_file(content_img_path)
            image_style_decoded = tf.read_file(style_img_path)
            nparr = np.fromstring(image_content_decoded.eval(), np.uint8)
            content_img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
            nparr = np.fromstring(image_style_decoded.eval(), np.uint8)
            style_img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
            s.close()
